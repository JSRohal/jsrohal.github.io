// Retro Terminal Portfolio — v8
// nav · clock · ASCII animations · project expand · contact animations · IP
...
function animateASCII() {
    ...
    ascii.innerHTML = ascii.textContent.split('\n')
        .map((l,i) => `<span class="ascii-line" style="--i:${i}">${escape(l)||'\u00a0'}</span>`)
        .join('');
}
function animateHomeSequence() {
    const asciiEndMs = 300 + 24 * 45;
    document.querySelectorAll('#home .boot-line').forEach((el,i) => {
        el.style.animationDelay = `${asciiEndMs + i * 210}ms`;
    });
    const heroStart = asciiEndMs + 5 * 210 + 80;
