...
function animateHomeSequence(asciiLineCount) {
    ...
    // Once every hero-fade element has finished its entry animation,
    // permanently disarm #home so switching tabs never replays the
    // boot sequence again — it only ever plays on the initial page load.
    const homeSection = document.getElementById('home');
    if (homeSection && heroFadeEls.length) {
        let finished = 0;
        const onDone = (e) => {
            if (e.target.classList.contains('hero-fade')) {
                finished++;
                if (finished >= heroFadeEls.length) {
                    homeSection.classList.remove('home-armed');
                    homeSection.removeEventListener('animationend', onDone);
                }
            }
        };
        homeSection.addEventListener('animationend', onDone);
    }
}
