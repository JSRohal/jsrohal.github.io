...
function animateHomeSequence(asciiLineCount) {
    ...
    const heroFadeEls = document.querySelectorAll('#home .hero-fade');
    heroFadeEls.forEach((el, i) => {
        el.style.animationDelay = `${greetingStart + i * 150}ms`;
    });

    // Once the whole boot/reveal choreography has finished playing,
    // mark the Home screen so the intro never replays on later tab
    // switches — only the always-on ascii color-wave keeps looping.
    const introTotalMs = greetingStart + heroFadeEls.length * 150 + 500;
    setTimeout(() => {
        const home = document.getElementById('home');
        if (home) home.classList.add('intro-played');
    }, introTotalMs);
}
...
const ABOUT_DATA = {
    profile: {
        ...
        paragraphs: [
            `I am a Mechanical Engineering student at Ohio University with a passion for design, manufacturing, software development, and problem solving. My interest in engineering began at an early age while helping my father with hands-on projects, learning how things were built, repaired, and improved. Since then, that curiosity has evolved into a drive to understand complex systems and create practical solutions through engineering and technology.`,
            `Beyond the classroom, I have participated in a variety of technical and leadership experiences including industry internships, student organizations, and collaborative design projects. These opportunities have strengthened my abilities in project management, CAD design, communication, problem solving, and cross-disciplinary teamwork.`,
            `I enjoy tackling challenging problems, learning new technologies, and working with teams that value innovation and continuous improvement. Whether designing systems, writing code, or leading projects, I am committed to delivering quality results.`
        ]
    },
    ...
    education: {
        ...
        gpaCurrent: 3.63,
        gpaMax: 4.00,
        // additional credentials shown as tree branches under the main degree
        credentials: [
            'Mathematics Minor',
    ...
    extracurriculars: [
        ...
        {
            org: 'Outdoors &amp; Fitness',
            role: 'Fishing &middot; Lifting &middot; Hiking',
            meta: 'Active Lifestyle &nbsp;&middot;&nbsp; Ongoing',
            desc: 'Stay active outside of the classroom through regular strength training, hiking, and fishing trips whenever the schedule allows.'
        }
    ]
};

// builds a "[||||||    ]" style meter bar string for a given ratio
function buildMeterBar(current, max, width) {
...
function renderProfile() {
    ...
    if (!el) return;
    el.innerHTML = ABOUT_DATA.profile.paragraphs
        .map(p => `<p class="paragraph">${p}</p>`)
        .join('');

    const img = document.getElementById('profilePhotoImg');
    const fallback = document.getElementById('profilePhotoFallback');
    if (img) {
        img.src = ABOUT_DATA.profile.photo;
        img.addEventListener('error', () => {
            img.style.display = 'none';
            if (fallback) {
                fallback.style.display = 'flex';
                const hint = fallback.querySelector('.ph-hint');
                if (hint) hint.textContent = `add image at ${ABOUT_DATA.profile.photo}`;
            }
        }, { once: true });
    }
}

function renderVtList(containerId, entries, { dotClass = '', showLoc = false, showSkills = false } = {}) {
    const el = document.getElementById(containerId);
    if (!el) return;
    el.innerHTML = entries.map((e, i) => {
        const skillsId = `skills-${containerId}-${i}`;
        const locHtml = showLoc && e.loc
            ? `<div class="exp-loc">&#9655;&nbsp;${e.loc}</div>` : '';
        const skillsHtml = showSkills && e.skills ? `
            <button class="skills-toggle" data-skills-target="${skillsId}" aria-expanded="false">$ view_skills&nbsp;&#9662;</button>
            <div class="skills-panel" id="${skillsId}">
                ${e.skills.map(s => `<span class="skill-tag">${s}</span>`).join('')}
            </div>` : '';
        return `
            <div class="vt-entry">
                <div class="vt-rail"><div class="vt-dot${dotClass ? ' ' + dotClass : ''}"></div></div>
                <div class="vt-body">
                    <div class="exp-org">${e.org}</div>
                    <div class="exp-role">${e.role}</div>
                    <div class="exp-meta">${e.meta}</div>
                    ${locHtml}
...
function renderExperience() {
    renderVtList('experienceList', ABOUT_DATA.experience, { showLoc: true, showSkills: true });
    const count = document.getElementById('experienceCount');
    if (count) count.textContent = `${ABOUT_DATA.experience.length} role${ABOUT_DATA.experience.length === 1 ? '' : 's'}`;
}

function renderExtracurriculars() {
    renderVtList('extracurricularsList', ABOUT_DATA.extracurriculars, { dotClass: 'vt-dot--cyan' });
    const count = document.getElementById('extracurricularsCount');
    if (count) count.textContent = `${ABOUT_DATA.extracurriculars.length} pursuit${ABOUT_DATA.extracurriculars.length === 1 ? '' : 's'}`;
}

function renderEducation() {
    const edu = ABOUT_DATA.education;

    const cardEl = document.getElementById('eduCard');
    if (cardEl) {
        cardEl.innerHTML = `
            <div class="edu-card-icon">&#9678;</div>
            <div class="edu-card-body">
                <div class="edu-degree-lg">${edu.degree}</div>
                <div class="edu-school-lg">${edu.school}</div>
                <div class="edu-gpa-meter">
                    <span class="edu-gpa-meter-label">GPA</span>
                    <span class="edu-gpa-meter-bar">${buildMeterBar(edu.gpaCurrent, edu.gpaMax, 18)}</span>
                    <span class="edu-gpa-meter-val">${edu.gpaCurrent.toFixed(2)} / ${edu.gpaMax.toFixed(2)}</span>
                </div>
            </div>`;
    }

    const credsEl = document.getElementById('eduCreds');
    if (credsEl) {
        credsEl.innerHTML = edu.credentials.map((label, i) => {
            const last = i === edu.credentials.length - 1;
            const arm = last ? '&#9492;&#9472;&#9472;' : '&#9500;&#9472;&#9472;';
            return `
                <div class="edu-cred${last ? ' edu-cred--last' : ''}">
                    <span class="tree-arm">${arm}</span>
...
function openProject(slug) {
    ...
    data.images.forEach((src, i) => {
        ...
        img.addEventListener('error', () => {
            ...
            ph.innerHTML = `
                <span class="ph-label">// screenshot ${String(i + 1).padStart(2, '0')}</span>
                <span class="ph-hint">add image at ${src}</span>`;
            wrap.appendChild(ph);
        }, { once: true });
    ...
    if (timelineWrapper) savedTimelineScrollLeft = timelineWrapper.scrollLeft;
...
function closeProject() {
    ...
    if (screen) screen.scrollTop = 0;
    const timelineWrapper = document.querySelector('.timeline-scroll-wrapper');
    if (timelineWrapper) timelineWrapper.scrollLeft = savedTimelineScrollLeft;
}
...
// ── Timeline scroll-wheel → horizontal scroll ────────────────────
function initTimelineWheelScroll() {
    const wrapper = document.querySelector('.timeline-scroll-wrapper');
    if (!wrapper) return;
...
// ── Color theme toggle (standard <-> retro) ──────────────────────
function initThemeToggle() {
    const root  = document.documentElement;
    const btn   = document.getElementById('themeToggle');
    const label = document.getElementById('themeToggleLabel');
    if (!btn) return;
    const STORAGE_KEY = 'jsr-portfolio-theme';
    ...
    function applyTheme(theme) {
        if (theme === 'retro') {
            ...
            if (label) label.textContent = 'Retro Mode';
        }
    }

    let saved = null;
    try { saved = localStorage.getItem(STORAGE_KEY); } catch (_) {}
    applyTheme(saved === 'retro' ? 'retro' : 'standard');

    btn.addEventListener('click', () => {
        const next = root.getAttribute('data-theme') === 'retro' ? 'standard' : 'retro';
        applyTheme(next);
...
// ── About tab nav wheel ────────────────────────────────────────
function initAboutWheel() {
    const wheel = document.querySelector('.nav-wheel');
    if (!wheel) return;
    const caption = document.getElementById('wheelCaption');
    const nodes = Array.from(wheel.querySelectorAll('.wheel-node'));
    const closeHub = document.getElementById('wheelClose');
    const views = Array.from(document.querySelectorAll('.about-view'));

    function activeNode() { return nodes.find(n => n.classList.contains('active')); }
    ...
    function resetAboutScroll() {
        ...
        if (aboutScreen) aboutScreen.scrollTop = 0;
    }

    function select(node) {
        nodes.forEach(n => n.classList.remove('active'));
        node.classList.add('active');
        showView(node.dataset.target);
        if (caption) caption.textContent = `MODULE: ${node.dataset.label}`;
        if (node.dataset.target === 'education') {
            if (!eduLogPlayed) { eduLogPlayed = true; typeEduLog(); }
            else { showEduLogInstant(); }
        }
        resetAboutScroll();
    }
    ...
    function clearSelection() {
        nodes.forEach(n => n.classList.remove('active'));
        showView('empty');
        if (caption) caption.textContent = 'SELECT A MODULE';
        resetAboutScroll();
    }
    ...
    nodes.forEach(node => {
        ...
        node.addEventListener('mouseenter', () => {
            if (caption) caption.textContent = `${node.classList.contains('active') ? 'MODULE' : 'PREVIEW'}: ${node.dataset.label}`;
        });
        node.addEventListener('mouseleave', () => {
            if (!caption) return;
            const active = activeNode();
            caption.textContent = active ? `MODULE: ${active.dataset.label}` : 'SELECT A MODULE';
        });
    });

    if (closeHub) {
        closeHub.addEventListener('click', clearSelection);
        closeHub.addEventListener('keydown', e => {
    ...
    document.addEventListener('click', e => {
        ...
        if (!btn) return;
        const panel = document.getElementById(btn.dataset.skillsTarget);
        if (!panel) return;
...
function buildEduLogLines() {
    const edu = ABOUT_DATA.education;
    return [
        '> querying academic_records.db ...',
        `> degree........... ${edu.degree.toUpperCase()}`,
        `> institution....... ${edu.school.toUpperCase()}`,
        `> gpa................ ${edu.gpaCurrent.toFixed(2)} / ${edu.gpaMax.toFixed(2)}`,
        '> status............. IN PROGRESS',
    ];
}

function showEduLogInstant() {
    const el = document.getElementById('eduLog');
    if (!el) return;
    eduLogGen++; // cancel any in-flight typewriter run
    el.textContent = buildEduLogLines().join('\n');
}

function typeEduLog() {
    const el = document.getElementById('eduLog');
    if (!el) return;
    const lines = buildEduLogLines();
    const gen = ++eduLogGen;
    el.textContent = '';
    let li = 0, ci = 0;
    function tick() {
        if (gen !== eduLogGen) return; // a newer run superseded this one
...
//          where the browser supports it (Chromium-based only; shown
//          as N/A elsewhere rather than guessed)
function initSystemMeters() {
    ...
    function renderBar(el, pct) {
        ...
        el.textContent = '[' + '|'.repeat(filled) + ' '.repeat(BAR_WIDTH - filled) + ']';
    }
    ...
    function frameLoop(now) {
        ...
        lastFrame = now;
        if (frameSamples.length > 40) frameSamples.shift();
    ...
    function updateCPU() {
        ...
        // ~16.7ms/frame is a smooth 60fps baseline; scale overage above that up to 100%
        const pct = Math.max(2, Math.min(100, ((avg - 16.7) / 40) * 100 + 6));
        renderBar(cpuBarEl, pct);
        if (cpuValEl) cpuValEl.textContent = `${Math.round(pct)}%`;
    }

    const conn = navigator.connection || navigator.mozConnection || navigator.webkitConnection;
    function updateNet() {
        if (!conn) {
            if (netBarEl) netBarEl.textContent = '[ N/A ]';
            if (netValEl) netValEl.textContent = '';
            return;
        }
        let pct = 50, label = '';
        if (typeof conn.downlink === 'number' && conn.downlink > 0) {
            pct = Math.max(4, Math.min(100, (conn.downlink / 10) * 100));
            label = `${conn.downlink.toFixed(1)}Mb`;
        } else if (conn.effectiveType) {
            const map = { 'slow-2g': 8, '2g': 22, '3g': 55, '4g': 92 };
            pct = map[conn.effectiveType] ?? 50;
            label = conn.effectiveType.toUpperCase();
        }
        renderBar(netBarEl, pct);
        if (netValEl) netValEl.textContent = label;
    }
...
if (ipEl || locTextEl || weatherTextEl) {
    ...
    async function fetchWeather(lat, lon, useFahrenheit) {
        ...
        const d = await getJSON(`https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&current_weather=true&temperature_unit=${unit}`);
        const cw = d && d.current_weather;
        if (!cw) throw new Error('no weather');
        const temp  = Math.round(cw.temperature);
        const label = WMO_LABELS[cw.weathercode] ?? 'Weather N/A';
        return `${temp}\u00B0${useFahrenheit ? 'F' : 'C'} \u00B7 ${label}`;
    }

    const providers = [
        async () => { const d = await getJSON('https://get.geojs.io/v1/ip/geo.json');
            return { ip:d.ip, parts:[d.city,d.region,d.country_code], lat:d.latitude, lon:d.longitude, country:d.country_code }; },
        async () => { const d = await getJSON('https://ipwho.is/');
            if (d.success===false) throw new Error('fail');
            return { ip:d.ip, parts:[d.city,d.region_code,d.country_code], lat:d.latitude, lon:d.longitude, country:d.country_code }; },
    ...
    (async () => {
        for (const get of providers) {
            try {
                const { ip, parts, lat, lon, country } = await get();
                if (!ip) throw new Error('no ip');
                const loc = parts.filter(Boolean).join(', ');
                if (ipEl) ipEl.textContent = `IP: ${ip}`;
                if (locTextEl) locTextEl.textContent = loc || 'Location unknown';

                if (weatherTextEl) {
                    if (lat != null && lon != null) {
                        try {
                            const useF = (country || '').toUpperCase() === 'US';
                            const wx = await fetchWeather(lat, lon, useF);
        ...
        if (locTextEl) locTextEl.textContent = 'Location unavailable';
        if (weatherTextEl) weatherTextEl.textContent = '// weather unavailable';
    })();
}
