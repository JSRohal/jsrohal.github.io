...
// ── Color theme toggle (professional <-> retro) ──────────────────
function initThemeToggle() {
    ...
    function applyTheme(theme) {
        if (theme === 'retro') {
            ...
            if (label) label.textContent = 'Professional Mode';
        } else {
    ...
    applyTheme(saved === 'retro' ? 'retro' : 'professional');

    btn.addEventListener('click', () => {
        const next = root.getAttribute('data-theme') === 'retro' ? 'professional' : 'retro';
        applyTheme(next);
